...
function breaker(distance, startPoint, endPoint , motion_type) {
  ...
  if (!isFinite(slope)) {
      ...
      for(let iterations = 1 ; iterations < sub_divisions ; iterations++){
          ...
          sub_divided_points.push({rise : sub_divided_rise , run : sub_divided_run})
...
function onSectionEnd() {
  ...
  for(counter = 1 ; counter < motion_list.length ; counter++){
    subdivided_motion_list.push(breaker(0.01 , motion_list[counter-1],motion_list[counter]))
  }

  for(counter = 0 ; counter < subdivided_motion_list.length ; counter++){
    writeln(subdivided_motion_list[counter].rise + ' '+ subdivided_motion_list[counter].run)

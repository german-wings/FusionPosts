...
function insertBlockNoOnToolChange(){
  if(typeof insertBlockNoOnToolChange.blockno == 'undefined'){
    insertBlockNoOnToolChange.blockno = 1
  }
...
//EDITS
//CHANGED COOLANT CODE FROM 8 TO 7 ON COOLANT_FLOOD VAR
var coolants = [
...
function onOpen() {
  ...
  //edits
  writeComment("Property of Shakti Enterprise")
  //edits
  writeComment("Vertraue auf den Herrn")
  //edits
  writeBlock("(Date -" + formatAMPM(new Date())+")")
      ...
      for (var i = 0; i < tools.getNumberOfTools(); ++i) {
        ...
        var comment = "T" + toolFormat.format(tool.number * 100 + compensationOffset % 100) + " " +
          ...
          (zRanges[tool.number] ? " - " + localize("ZMIN") + "=" + spatialFormat.format(zRanges[tool.number].getMinimum()) : "") +
           ...
           //edits
          let tool_no = "T"+toolFormat.format(tool.number*100+compensationOffset%100)
          let tool_name = tool.description
          var tool_z_max = spatialFormat.format(
            zRanges[tool.number].getMinimum()
          );
          writeBlock("("+tool_no+")("+tool_name+"|MAX Z DEPTH "+tool_z_max+")")
      }
    }
  }
...
function onSection() {
  ...
  if (hasParameter("operation-comment")) {
    ...
    if (comment) {
      ...
      //edits
      writeComment("Operation Uses "+tool.description)
    }
  }
  ...
  if (insertToolCall) {
    ...
    //edits
    //add N block on tool change
    writeBlock("N"+insertBlockNoOnToolChange())
